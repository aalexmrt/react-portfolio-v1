...
export const NavBar = () => {
 ...
 return (
  <nav className="navbar flex items-center justify-between text-[0.8em] md:grid md:grid-cols-3 md:px-0 md:text-[1rem] ">
   <NavLink className={({ isActive }) => (isActive ? 'null' : 'null')} to="/">
   ...
   <ul className="nav justify-self-center rounded-full p-1 font-medium shadow-sm drop-shadow-sm md:flex md:justify-between md:gap-4 md:border-2 md:bg-[#fffcfc] md:px-8 md:py-2">
    {NAVSECTIONS
        ...
        return (
         <Fragment key={index}>
          ...
          {NAVSECTIONS.length - 1 !== index ? (
           <span className="hidden h-5 w-[1px] self-center bg-neutral-400 md:block"></span>
          ) : null}
         </Fragment>
        )
       })
